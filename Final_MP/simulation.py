...
susceptibility = np.array([1, 1, 1])  # paper also uses 1 ## sigma

infectivity = np.array([1, 1, 1])  # paper also uses 1 ## eta

initially_infected = np.array([1, 2, 4])  # Initial infected individuals in each group #MP# Zašto ovakav izbori a ne npr. [1,1,1] ili [0,1,0] ili [1,0,0] ili [0,0,1]? Čini li to razliku za rezultate?

# ---------------------------------------- User Input Parameters ---------------------------------------------

population = int(input("Enter population size: "))

reproduction_number = float(input("Enter R0: ")) #MP# zar ovo ne računamo kao spektralni radijus matrice M_ij?

recovery_rate = float(input("Enter recovery rate, 1/(duration of infection): ")) #MP# gamma
...
for j in range(len(contact_matrix)):
    for k in range(len(contact_matrix)):
        next_generation_matrix[j, k] = (susceptibility[j] * infectivity[k] * contact_matrix[j, k]) / \
                                       group_size_fractions[k] #MP# ovo nije next_generation_matrix već matrica D_jk
#MP# next_generation_matrix M_jk se malo drugačije definira pogledajte formulu na str. 7 odmah ispod (6.2)


spectral_radius = max(np.linalg.eigvals(next_generation_matrix).real)

transmission_rate = (reproduction_number * recovery_rate) / spectral_radius #MP# Odakle smo ovo dobili? Zar ne bi taj beta(?) trebao biti parametar modela?
    ...
    for t in range(1, days):
        for j in range(num_groups):
            ...
            force_of_infection = transmission_rate * sum(
                contact_matrix[j, k] * I[t - 1, k] / group_sizes[k] for k in range(num_groups)
            ) #MP# Ajde molim Vas ovdje definirajte matricu D_ij kao što je to napravljeno i u modelu pa za nju zapišite jednadžbe.

            # Changes for group j
            dS = -force_of_infection * S[t - 1, j] #MP# matrica D_ij
            dV = -force_of_infection * (1 - vaccine_efficacy) * V[t - 1, j]  # Partial susceptibility #MP# matrica D_ij
            dI = force_of_infection * (S[t - 1, j] + (1 - vaccine_efficacy) * V[t - 1, j]) - recovery_rate * I[t - 1, j] #MP# matrica D_ij
            dR = recovery_rate * I[t - 1, j]
...
#MP# Ovdje, ako dobro shvaćam, radite diskretizaciju vremena u dane. Bilo bi dobro da kao i u prvim verzijama koda iskoristite scipy.integrate.odeint i vidite što on vraća i usporedite. S tim da bi bilo dobro isprobati i finiju mrežu od one jednodnevne. Konkretno za t koji prosljeđujete funkciji odeint bi mogli staviti t = np.linspace(0, t_max, t_max*1000) npr. pa vidjeti ima li razlike (probajte i za jednodnevni).
